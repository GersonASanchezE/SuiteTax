...
||                                                              ||
||  File Name: LMRY_CO_ST_Bill_WHT_Lines_LBRY_V2.0.js           ||
||                                                              ||
||  Version Date         Author        Remarks                  ||
||  2.0     Dic 01 2021  LatamReady    Use Script 2.0           ||
 \= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = */
...
function (log, format, record, search, runtime, Library_Mail, Library_Log) {
  var LMRY_SCRIPT = "LatamReady - CO ST Bill WHT Lines LBRY";
  var LMRY_SCRIPT_NAME = 'LMRY_CO_ST_Bill_WHT_Lines_LBRY_V2.0.js';
              ...
              if ( CC_generatedTransactionID == "5" ) {
                var auxTransJson = {
                  ...
                  nationalTax: "",
                    ...
                    expenseUniqueKey: expenseUniqueKey,
                    department: CC_DepartmentID,
                    class: CC_ClassID,
                    location: CC_LocationID,
                    whtamount: Math.round(WHTAmount * 100) / 100,
          ...
          if (expenseLineCount != null && expenseLineCount > 0) {
            ...
            for (var j = 0; j < expenseLineCount; j++) {
              ...
              if (Library_Mail.getAuthorization(349, licenses) == true) {
                ...
                for (var whtdetail in transactionArray) {
                  if ((transactionArray[whtdetail].subtype.value == NT_subtypeID) 
                  && (transactionArray[whtdetail].appliestoaccount == NT_creditAccountID)
                  && (transactionArray[whtdetail].expenseUniqueKey == expenseUniqueKey)) {
                      avoidNT = true;
                      break;
                    }
                }
                ...
                for (var whtdetail in journalArray) {
                  if ((journalArray[whtdetail].subtype.value == NT_subtypeID) 
                  && (journalArray[whtdetail].appliestoaccount == NT_creditAccountID)
                  && (journalArray[whtdetail].expenseUniqueKey == expenseUniqueKey)) {
                    avoidNT = true;
                    break;
                  }
                }
