...
function customizeGlImpact(transactionRecord, standardLines, customLines, book) {
    ...
    try {
        ...
        if (resultSearchSub.length != null && resultSearchSub.length > 0) {
            ...
            taxCalculationForm = resultSearchSub[0].getValue("custrecord_lmry_setuptax_br_taxfromgross");
        ...
        if (salesTransactionTypes.indexOf(transactionType) != -1) {
            ...
            if (searchResultCC != null && searchResultCC.length > 0) {
                ...
                for (var i = 0; i < searchResultCC.length; i++) {
                    ...
                    var class = searchResultCC[i].getValue("custrecord_lmry_ar_ccl_class");
                    ...
                    if (applies_to == 1) {
                        ...
                        if (parseFloat(retencion) > 0) {
                            var retencionAuxRound = typeRounding(retencion, rounding);
                            var retencionAuxRoundBR = retencionAuxRound;
                            if (featureMB) {
                            ...
                            if (parseFloat(retencionAuxRound) > 0) {
                                ...
                                if (country.toUpperCase() == 'BRA' && (is_exclusive != true && is_exclusive != 'T') && telecommTaxesFlows.indexOf(Number(taxCalculationForm)) != -1) {
                                    sumaBaseCalculoT = sumaBaseCalculoT + parseFloat(retencionAuxRoundBR);
                                }
                                ...
                                if (clasificacionCC == true || clasificacionCC == 'T') { // Segmentacion de la Clase Contributiva
                                    ...
                                    var cla1 = elegirSegmentacion(prefClas, class, transactionRecord.getFieldValue("class"), classSetup, transactionRecord.getFieldValue("class"));
                                    ...
                                    var cla1 = elegirSegmentacion(prefClas, transactionRecord.getFieldValue("class"), class, classSetup, '');
                    ...
                    // Caso 2I / 2E : Entidad - Items / Expenses
                        ...
                        if ((country.toUpperCase() != 'BRA' && country.toUpperCase() != 'ECU' && applies_item != null && applies_item != '') ||
                            ...
                            for (var j = 1; j <= cantidad_items; j++) {
                                ...
                                switch (country.toUpperCase()) {
                                    case 'BRA':
                                        catalogItem = transactionRecord.getLineItemValue("item", "custcol_lmry_br_service_catalog", j);
                                        ...
                                        break;
                                    case 'ECU':
                                        catalogItem = transactionRecord.getLineItemValue("item", "custcol_lmry_ec_concept_ret", j);
                                    ...
                                    if (parseFloat(retencion) > 0) {
                                        ...
                                        if (parseFloat(retencionAuxRound) > 0) {
                                            ...
                                            if ((is_exclusive != true && is_exclusive != 'T') && telecommTaxesFlows.indexOf(Number(taxCalculationForm) != -1)) {
                                                baseCalculoBR(parseFloat(retencionAuxRoundBR), j, true);
                                            ...
                                            if (clasificacionCC == true || clasificacionCC == 'T') { // Segmentacion de la CC
                                                ...
                                                arreglo_DetailsItem.push(elegirSegmentacion(prefClas, class, transactionRecord.getLineItemValue("item", "class", j), classSetup, transactionRecord.getFieldValue("class")));
                                                ...
                                                arreglo_DetailsItem.push(elegirSegmentacion(prefClas, transactionRecord.getLineItemValue("item", "class", j), class, classSetup, ''));
                        ...
                        if (filtroTransactionType == 4 || filtroTransactionType == 7) { // Bill - Bill Credit
                            if ((country.toUpperCase() != 'ECU' && applies_account != null && applies_account != '') || (country.toUpperCase() == 'ECU' && catalog_ec != null && catalog_ec != '')) {
                                for (var j = 1; j <= cantidad_expenses; j++) {
                                    var idAccount = transactionRecord.getLineItemValue("expense", "account", j);
                                    var catalogExpense = "";
                                    var flagExpense = false;

                                    switch (country.toUpperCase()) {
                                        case 'ECU':
                                            catalogExpense = transactionRecord.getLineItemValue("expense", "custcol_lmry_ec_concept_ret", j);
                                    ...
                                    if (flagExpense == true) {
                                        var grossamtItem = transactionRecord.getLineItemValue("expense", "grossamt", j);
                                        var taxamtItem = 0.00;
                                        if (FEAT_SUITETAX == "T" || FEAT_SUITETAX == true) {
                                            taxamtItem = transactionRecord.getLineItemValue("expense", "taxamount", j);

                                        } else {
                                            taxamtItem = transactionRecord.getLineItemValue("expense", "tax1amt", j);
                                        }
                                        var netamtItem = round2(parseFloat(grossamtItem) - parseFloat(taxamtItem));
                                        ...
                                        if (parseFloat(retencion) > 0) {
                                            ...
                                            if (parseFloat(retencionAuxRound) > 0) {
                                                ...
                                                var arreglo_DetailsItem = new Array();
                                                arreglo_DetailsItem.push('E');
                                                arreglo_DetailsItem.push(idsubtype);
                                                arreglo_DetailsItem.push(subtype);

                                                if (clasificacionCC == true || clasificacionCC == 'T') { // Segmentacion de la CC
                                                    arreglo_DetailsItem.push(elegirSegmentacion(prefDep, department, transactionRecord.getLineItemValue("expense", "department", j), depSetup, transactionRecord.getFieldValue("department")));
                                                    arreglo_DetailsItem.push(elegirSegmentacion(prefClas, class, transactionRecord.getLineItemValue("expense", "class", j), classSetup, transactionRecord.getFieldValue("class")));
                                                    arreglo_DetailsItem.push(elegirSegmentacion(prefLoc, location, transactionRecord.getLineItemValue("expense", "location", j), locSetup, transactionRecord.getFieldValue("location")));
                                                } else { // Segmentacion del Item
                                                    arreglo_DetailsItem.push(elegirSegmentacion(prefDep, transactionRecord.getLineItemValue("expense", "department", j), department, depSetup, ''));
                                                    arreglo_DetailsItem.push(elegirSegmentacion(prefClas, transactionRecord.getLineItemValue("expense", "class", j), class, classSetup, ''));
                                                    arreglo_DetailsItem.push(elegirSegmentacion(prefLoc, transactionRecord.getLineItemValue("expense", "location", j), location, locSetup, ''));
                                                }
                                                ...
                                                arreglo_DetailsItem.push(catalogExpense); // Catalogo BR - EC

                                                arreglo_DetailsItem.push(retencionAuxRound);
                                                arreglo_DetailsItem.push(' (LatamTax - Contributory Class) - (ID Account Expense: ' + idAccount + ')');
            ...
            if (searchResultNT != null && searchResultNT.length > 0) {
                ...
                for (var i = 0; i < searchResultNT.length; i++) {
                    ...
                    var class = searchResultNT[i].getValue("custrecord_lmry_ntax_class");
                    ...
                    // Caso 4I / 4E : Subsidiaria - Items / Expenses
                                    ...
                                    if (flagExpense == true) {
                                        ...
                                        if (parseFloat(retencion) > 0) {
                                            ...
                                            if (parseFloat(retencionAuxRound) > 0) {
                                                ...
                                                arreglo_DetailsItem.push(catalogExpense); // Catalogo BR - EC
                                                arreglo_DetailsItem.push(retencionAuxRound);
                                                arreglo_DetailsItem.push(' (LatamTax - National Tax) - (ID Account Expense: ' + idAccount + ')');
                        ...
                        } // Fin Caso 4E : Subsidiaria - Expenses
                    } // Caso 4I / 4E : Subsidiaria - Items / Expenses
                } // Fin for NT
            } // Fin if NT != null
...
function tipoCambioMB(transactionRecord, book, retencionMB, rounding) {
    ...
    retencionMB = typeRounding(retencionMB, rounding);
    ...
    if (FEAT_CUSTOMSEGMENTS == true || FEAT_CUSTOMSEGMENTS == "T") {
        ...
        if (segment_ids.length) {
            ...
            for (var i = 0; i < segment_ids.length; i++) {
                filterSegments.push("OR", ["scriptid", "startswith", segment_ids[i]])
            }
...
function getTaxCalculatorConfigTaxes(transactionRecord) {
    var configTaxes = {}
    var typeTransactions = { "sales": ["invoice", "creditmemo", "salesorder"], "purchase": ["vendorbill", "purchaseorder"] };
