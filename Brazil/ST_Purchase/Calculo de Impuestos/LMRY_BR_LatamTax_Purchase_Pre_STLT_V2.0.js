...
define(['N/runtime', 'N/ui/serverWidget', 'N/log', 'N/redirect', 'N/url',
    './Latam_Library/LMRY_BR_LatamTax_Purchase_LBRY_V2.0',
    './Latam_Library/LMRY_BR_ST_LatamTax_Purchase_LBRY_V2.0'
  ],
  function(runtime, serverWidget, log, redirect, url, libraryTaxPurchase, ST_TaxPurchase) {
      ...
      try {
        ...
        if (scriptContext.request.method == 'GET') {
          ...
          if (ST_FEATURE === true || ST_FEATURE === "T") {
            JsonResult = ST_TaxPurchase.getTaxPurchase(idTransaction, typeTransaction);
          } else {
            JsonResult = libraryTaxPurchase.getTaxPurchase(idTransaction, typeTransaction);
          }
          ...
          for (var key in JsonResult) {
            for (var i = 0; i < JsonResult[key].length; i++) {
              ...
              var position = parseInt(JsonResult[key][i].posicionItem) + 1;
              if (position) {
                SubTabla.setSublistValue({
                  id: 'id_pos',
                  line: cont,
