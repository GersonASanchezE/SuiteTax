/* = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =\
  ...
  ||                                                              ||
  ||  File Name:  LMRY_ContributoryClass_URET_V2.0.js  	          ||
  ||                                                              ||
...
define(['N/log', 'N/ui/serverWidget', 'N/runtime', 'N/search', 'N/record', './Latam_Library/LMRY_libSendingEmailsLBRY_V2.0', './Latam_Library/LMRY_Log_LBRY_V2.0',
  ...
  function (log, serverWidget, runtime, search, record, libraryMail, libLog, ST_CC_NT_Library) {
    var LMRY_script = "LatamReady - Contributory Class URET V2.0";
    ...
    function beforeLoad(context) {
      try {
        ...
        if (type == 'view') {
          ...
          showFieldsFromCheckBox(recordObj, viewFields, "custrecord_lmry_br_ccl_rate_suma", "custrecord_lmry_br_exclusive");
          showFieldsFromCheckBox(recordObj, viewFields, "custrecord_lmry_ccl_reduction_ratio", "custrecord_lmry_ccl_is_reduction");
    ...
    function afterSubmit(context) {
        ...
        try {
            ...
            if (ST_FEATURE == true || ST_FEATURE == "T") {
                ST_CC_NT_Library.setTaxTypeAndTaxCodeBYCC(recordObj);
            }
    ...
    function getViewFields(recordObj) {
      ...
      var country = recordObj.getValue({
        fieldId: 'custrecord_lmry_ccl_subsidiary_country'
      });
      ...
      if (country && taxtype && gentrans) {
        ...
        var filters = [
          ['isinactive', 'is', 'F'], 'AND',
          ['custrecord_lmry_setuptaxfield_record', 'is', '1'], 'AND',
    ...
    function getFieldsToHide(recordObj) {
      var results = [];
      var fixedFields = [
        'custrecord_lmry_ar_ccl_subsidiary', 'custrecord_lmry_ccl_subsidiary_country', 'custrecord_lmry_ar_ccl_entity',
        'custrecord_lmry_ccl_transactiontypes', 'custrecord_lmry_ccl_gen_transaction', 'custrecord_lmry_ccl_taxtype']
    ...
    function setTaxRate(recordObj) {
      ...
      var difalTaxRate = recordObj.getValue({
        fieldId: "custrecord_lmry_ccl_difal"
      });

      var difalTaxRatePrct = recordObj.getValue({
        fieldId: "custrecord_lmry_ccl_difal_pctge"
      });
    ...
    function showBRLocationFields(recordObj, viewFields) {
      var country = recordObj.getValue('custrecord_lmry_ccl_subsidiary_country');
      var subtype = recordObj.getValue('custrecord_lmry_sub_type');
      ...
      if (Number(country) == 30 && subtype) { //Brazil
        ...
        if (isTaxByLocation) {
          viewFields.push("custrecord_lmry_ccl_province", "custrecord_lmry_ccl_city", "custrecord_lmry_ccl_district");
        }
      }
    }
        ...
        try {
          ...
          if (Object.keys(segmentRecords).length) {
            ...
            for (var s in segmentRecords) {
              ...
              form.insertField({
                field: field,
                nextfield: "custrecord_lmry_ccl_config_segment"
              });
            }
          }
    ...
    function showCustomSegmentFields(recordObj, viewFields, customSegments) {
      ...
      if (Number(country) == 30 && [2, 6].indexOf(Number(gentrans)) != -1 && [1, 4].indexOf(Number(taxtype)) != 1) { //Calculo de Impuestos y Retenciones BR
    ...
    function setCustomSegments(recordObj, form, customSegments) {
      var stringObjSegments = recordObj.getValue({
        fieldId: "custrecord_lmry_ccl_config_segment"
      });
    ...
    function createBrTelecomCatalogField(recordObj, form) {
      try {
        var catalogField = form.addField({
          id: "custpage_br_ccl_nfce_catalog",
        ...
        form.insertField({
          field: catalogField,
          nextfield: "custrecord_lmry_ccl_applies_to_account"
        });

        var telecomCatalog = recordObj.getValue("custrecord_lmry_ccl_nfce_catalog_id");
        if (telecomCatalog) {
