...
||                                                              ||
||  File Name: LMRY_MX_ST_Bill_WHT_Total_LBRY_V2.0.js           ||
||                                                              ||
...
function (log, format, record, search, runtime, cache, Library_Mail, Library_Log) {
  var LMRY_SCRIPT = "LatamReady - MX ST Bill WHT Total LBRY";
  var LMRY_SCRIPT_NAME = 'LMRY_MX_ST_Bill_WHT_Total_LBRY_V2.0.js';
    ...
    try {
      ...
      if (appliedWHT == true || appliedWHT == "T") {
        ...
        if (approvalFeature == true || approvalFeature == 'T') {
          ...
          if (approvalStatus && Number(approvalStatus) == 2) {
            ...
            Library_Mail.sendemail('[ setWHTTransaction]: The appoval status for the transaction is not "Approved". ', LMRY_SCRIPT);
            Library_Log.doLog({ title: '[ afterSubmit - setWHTTransaction ]', message: 'The appoval status for the transaction is not "Approved".', relatedScript: LMRY_SCRIPT_NAME });
  ...
  /***************************************************************************
  * FUNCION PARA CALCULAR LOS IMPUESTOS DE TIPO "RETENCION" 
  * GENERAR TAX RESULT
  ...
  *    - transactionArray: Informacion pora los Bill Credits
    ...
    try {
      if (CC_SearchResult != null && CC_SearchResult.length > 0) {
        for (var i = 0; i < CC_SearchResult.length; i++) {
          ...
          var total_bill = parseFloat(recordObj.getValue("total")) * parseFloat(exchangeRate);
          var taxtotal_bill = parseFloat(recordObj.getValue("taxtotal")) * parseFloat(exchangeRate);
          var subtotal_bill = total_bill - taxtotal_bill;
          var amount_base = 0;

          if (CC_appliesTo == '1') { // total
            if (CC_amountTo == 1) { // GROSS 
              amount_base = total_bill; 
            } 
            if (CC_amountTo == 2) { // TAX TOTAL
              if (taxtotal_bill > 0) {
                amount_base = taxtotal_bill;
              } else {
                continue;
              }
            }
            if (CC_amountTo == 3) { // NET 
              amount_base = subtotal_bill; 
            }
          }
        ...
        if (NT_SearchResult != null && NT_SearchResult.length > 0) {
          for (var i = 0; i < NT_SearchResult.length; i++) {
            ...
            var total_bill = parseFloat(recordObj.getValue("total")) * parseFloat(exchangeRate);
            var taxtotal_bill = parseFloat(recordObj.getValue("taxtotal")) * parseFloat(exchangeRate);
            var subtotal_bill = (total_bill - taxtotal_bill) * parseFloat(exchangeRate);
            var amount_base = 0;
    ...
    try {
      ...
      if (CONFIG_TRANSACTION[recordType]) {
        ...
        for (var subtypeID in groupsInformationWht) {
          ...
          log.error("ID NEW VENDOR CREDIT", idNewRecord);
        }
      }
    }
    catch (error){
