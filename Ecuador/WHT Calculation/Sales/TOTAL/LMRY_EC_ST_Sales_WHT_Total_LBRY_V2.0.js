...
||                                                              ||
||  File Name: LMRY_EC_ST_Sales_WHT_Total_LBRY_V2.0.js          ||
||                                                              ||
||  Version Date         Author        Remarks                  ||
||  2.0     Jan 20 2021  LatamReady    Use Script 2.0           ||
 \= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = */
/**
 * @NApiVersion 2.0
 * @NModuleScope Public
...
define([
  ...
  function mapeo(currentTran, jsonAmounts, NT) {
    var whyValue = currentTran.getValue("custbody_lmry_apply_wht_code");
    // PARA TOTALES DEBE TENER EL WHT CHECK ACTIVO
    if (!whyValue) return;

    if (scriptType != "suitelet") {
      // TODO: Buscar informacion de los
      // National Taxes relacionados a la transacción
      resultsNTbyWHT = searchNTbyWHT(currentTran.getValue("subsidiary"), NT);
      ...
      //REINICIA EL JSON NECESARIO CUANDO ES MAP REDUCE
      jsonGlobal = {};
    } else {
  ...
  function logicaLlenado(resultsNTbyWHT, currentTran, jsonAmounts) {
    ...
    if (resultsNTbyWHT && resultsNTbyWHT.length) {
      for (var i = 0; i < resultsNTbyWHT.length; i++) {
        ...
        var NT_IVA_EC = currentTran.getValue({
          name: "custrecord_lmry_ec_rte_iva",
          join: "CUSTRECORD_LMRY_EC_RELATED_TRANSACTION",
        });
        var NT_FTE_EC = currentTran.getValue({
          name: "custrecord_lmry_ec_rte_fte",
          join: "CUSTRECORD_LMRY_EC_RELATED_TRANSACTION",
        });

        // TODO: VALIDAR SI SE USA 1 DE LOS NT RELACIONADOS A LA TRANSACCION
        if (internalidNT != NT_IVA_EC && internalidNT != NT_FTE_EC) continue;
      ...
      for (var idTranJG in jsonGlobal) {
        for (var j = 0; j < jsonGlobal[idTranJG]["wht"].length; j++) {
          whtResult.push({
            ...
            basetotalamount: jsonGlobal[idTranJG]["wht"][j]["baseamount"],
            whttotalamount: jsonGlobal[idTranJG]["wht"][j]["whtamount"],
  ...
  function searchNTbyWHT(subsidiary, NT) {
    try {
      var NT_Columns = [
      ...
      var searchNT = search
        ...
        .run()
        .getRange(0, 10);
      log.debug("searchNTbyWHT : searchNT", searchNT);
      return searchNT;
    } catch (error) {
