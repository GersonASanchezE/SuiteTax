...
||                                                              ||
||  File Name: LMRY_EC_ST_Purchase_Tax_Transaction_LBRY_V2.0.js	||
||                                                              ||
||  Version Date          Author        Remarks                 ||
||  2.0     DIC 21 2021   LatamReady    Use Script 2.0          ||
 \= = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = */
...
define([
  ...
  var LMRY_SCRIPT = "LMRY - EC ST Purchase Tax Transaction V2.0";
  var LMRY_SCRIPT_NAME = "LMRY_EC_ST_Purchase_Tax_Transaction_LBRY_V2.0.js";
  ...
  function disableInvoicingIdentifier(scriptContext) {
    try {
      ...
      var typeForm = scriptContext.newRecord.getValue({
        fieldId: "customform",
      });
      log.debug("typeForm", "typeForm:" + typeForm);
      // 50 bill standard form
      // 48 bill credit standard form
      if (typeForm === 50 || typeForm === 48) {
        ...
        for (var i = 0; i < CC_SearchResult.length; i++) {
          ...
          if (itemLineCount != null && itemLineCount > 0) {
            for (var j = 0; j < itemLineCount; j++) {
              ...
              // Select Item Type
              var itemType = recordObj.getSublistText({
                sublistId: "item",
                fieldId: "itemType",
                line: j,
              });
              ...
              if (
                ...
                log.debug(
                  "[Si no se selecciona una opcion de impuestos item]",
                  j
                );
                continue;
              }
              if (itemType == "Group" || itemType == "EndGroup") {
                log.debug("GROUP ITEM END GROUP", itemType);
                continue;
              }
          ...
          if (expenseLineCount != null && expenseLineCount > 0) {
            for (var j = 0; j < expenseLineCount; j++) {
              ...
              if (
                ...
                // Se pasa al siguiente item
                log.debug(
                  "[Si no se selecciona una opcion de impuestos expense]",
                  j
                );
                continue;
              }
  ...
  function _getSetupTaxSubsidiary(subsidiary) {
    try {
      ...
      log.debug({
        title: "setupTaxSubsidiarySearch",
        details: setupTaxSubsidiarySearch[0],
      });
